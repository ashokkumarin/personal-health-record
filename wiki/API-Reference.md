...
a new family (see [Data model](Data-Model#family)).
...
[Data model → Authorization model](Data-Model#authorization-model) for the
full table.
...
[C3 — Component](C3-Component#thumbnail-generation-is-synchronous-ocr-is-not)).
...
[C3 — filesRoutes](C3-Component#filesroutes) for why).
...
not the JWT — see [Data model → System admin](Data-Model#system-admin-userisadmin-vs-familyrole)).
...
[Data model → Admin-initiated user deletion](Data-Model#admin-initiated-user-deletion-is-a-soft-delete-and-only-ever-touches-the-targets-own-data)
