...
Implements [Requirements and Project Plan](Requirements-and-Project-Plan) Section 6.2 (full model, not [MVP Spec](MVP-Spec)'s narrower version) — family ownership/admins, and all three ways of adding a family member.
...
- Mobile UI for the three-mode add-member flow and the approvals screen — mobile gets list/detail screens only this slice (see [Technical Design](Technical-Design)).
