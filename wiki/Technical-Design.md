...
This document translates [Requirements and Project Plan](Requirements-and-Project-Plan) and [MVP Spec](MVP-Spec) into a concrete architecture. It is the shared reference for every slice spec in [Feature Slices](Feature-Slices). Sections below describe the as-built system; each slice doc has the authoritative acceptance criteria for the piece it introduced.
...
> — see [Architecture Overview](Architecture-Overview). This document remains as the
...
- No refresh tokens or password reset (requirements doc Section 6.1 mentions these; still deferred — see [Slice 01 — Authentication](Slice-01-Authentication)).
...
See the full [API Reference](API-Reference) for every route.
...
- `apps/api`: Vitest, using Fastify's `.inject()` against a real test Postgres (via docker-compose) — no mocked DB, per the acceptance-criteria-first approach in [MVP Spec](MVP-Spec) Section 7.
