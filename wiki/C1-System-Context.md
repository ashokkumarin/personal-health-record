...
[Deployment](Deployment)). The **only** external dependency the system
context needs to show is the mobile app's use of the publicly-hosted `pdf.js`
viewer for in-app PDF preview — see
[Mobile app → PDF viewing](Mobile-App#pdf-viewing) for why that
...
this level. See [C3 — Component](C3-Component) and the
[data model](Data-Model) for those details.
