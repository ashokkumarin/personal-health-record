...
- API and web Docker images rebuilt and the running containers force-recreated (image rebuild alone doesn't recreate a running container — see [Deployment](Deployment#rebuilding-after-code-changes)); `prisma migrate deploy` applied.
- Documentation updated: this slice doc, [Data Model](Data-Model), [API Reference](API-Reference), [Web App](Web-App), [Mobile App](Mobile-App), [Backend Service](Backend-Service), [Deployment](Deployment), root README, and the `docs/` mirrors of each.
