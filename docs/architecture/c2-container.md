...
[Web app guide](../apps/web-app.md#session--auth)). Every API call goes through
...
[C3](c3-component.md#filesroutes) and [API reference → Files](../api-reference.md#files)).
...
[Mobile app guide](../apps/mobile-app.md).
...
See [C3 — Component](c3-component.md) for its internals and
[Backend service guide](../apps/backend-service.md).
...
`apps/api/prisma/`. See [Data model](../data-model.md).

### Media Storage
A local filesystem directory (`MEDIA_ROOT`, mounted as a named Docker volume
`phr-media` in production — see [Deployment](../deployment.md)). Not a separate
...
authentication (see [C3 — filesRoutes](c3-component.md#filesroutes)).
