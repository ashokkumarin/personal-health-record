...
[API reference → Conventions](API-Reference#conventions)), so callers do:
```ts
catch (err) {
...
```
Each method maps 1:1 onto a route in [API Reference](API-Reference) —
...
`zod` library (see [API reference → Conventions](API-Reference#conventions),
...
[C4 — Code](C4-Code) for the Records example):
